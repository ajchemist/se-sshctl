...
- resident wrapper installation matched the CTK SSH fingerprint;
...
- after removing the canary authorization, the same wrapper was rejected;
...
1. `ssh-keygen -K` required a non-empty dummy PIN (`0`) even for a `none`
   identity, followed by an empty wrapper passphrase.
2. `KEYCHAIN_CERTIFICATES` did not filter the security-key resident download.
   ...
   downloads isolated overwrite positions and accepts only the wrapper whose SSH
