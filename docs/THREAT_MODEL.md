...
| Identity-file swap in its directory | Files are installed 0400/0444; new directories are 0700 and an existing directory is refused when group- or world-writable | The identity file is a handle, so a readable directory discloses nothing the `.pub` does not; a same-user process is outside this boundary |
