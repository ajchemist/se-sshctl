...
locators and labels, SSH public keys/fingerprints, wrapper references, and remote
SSH authorization.
...
1. `se-sshctl` invokes fixed absolute Apple system executables with argument
   arrays. Resident-wrapper download additionally re-executes the current native
...
| Wrong-key wrapper installation | Separate SHA-256/SHA-1/SSH identifiers; isolated overwrite-position downloads plus fingerprint matching; ambiguous metadata is rejected | Large or mixed-token inventories need more physical-Mac coverage |
| Accidental identity deletion | Native SHA-1 hash plus exact confirmation for plumbing; SHA-256 selection plus remote-revocation/recovery acknowledgements for porcelain retirement; post-delete absence check; no bulk delete | Raw plumbing can be misused, and the CLI cannot independently prove every remote authorization was removed |

## Unverified behaviors

The `p-256-ne + none` canary covers creation, multiple-identity wrapper selection,
